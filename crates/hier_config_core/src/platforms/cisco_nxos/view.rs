//! View implementation for Cisco NX-OS.
...
use crate::view::helpers::{
    children_startswith, default_module_number, default_parent_name, parse_ipv4_interface,
};
...
/// Tree view logic for Cisco NX-OS.
#[derive(Debug)]
pub struct HConfigViewCiscoNxos<'a> {
    pub tree: &'a Tree,
}

impl<'a> HConfigViewCiscoNxos<'a> {
    #[must_use]
...
/// Interface view logic for Cisco NX-OS.
#[derive(Debug)]
pub struct ConfigViewInterfaceCiscoNxos<'a> {
    pub tree: &'a Tree,
    pub node_id: NodeId,
}

impl<'a> ConfigViewInterfaceCiscoNxos<'a> {
    #[must_use]
    ...
    pub fn ipv4_interfaces(&self) -> Result<Vec<(Ipv4Addr, u8)>, &'static str> {
        ...
        for text in children_startswith(self.tree, self.node_id, "ip address ") {
            let words: Vec<&str> = text.split_whitespace().collect();
            let Some(spec) = words.get(2) else { continue };
            if let Some(iface) = parse_ipv4_interface(spec, words.get(3).copied()) {
                ifaces.push(iface);
            }
        }
        Ok(ifaces)
