...
from hier_config import constants as C
...
#    def tags(self):
#        """
#        Finds tags of self and all children
#        interface eth1/1 [x]
#          hsrp 1 [x]
#            authentication something [x]
#
#        interface eth1/1 []
#          hsrp 1 []
#            authentication something [x]
...
#        interface eth1/1 [] +++ T
#          hsrp 1 [] --- T
#            authentication something [x] --- L
#          no ip address [x] ---- L
#        """
#        mytags = set()
#        #without support for trunk tags
#        #if self._is_leaf:
#        #    mytags.update(self._tags)
#        #with support for trunk tags
#        mytags.update(self._tags)
#        else:
#            for child in self.all_children():
#                #with support for trunk tags
#                mytags.update(child._tags)
#                #without support for trunk tags
#                #if self._is_leaf:
#                #    mytags.update(child._tags)
#        return mytags
    ...
    def get_child_deep(self, test_expression_pairs):
        ...
        Returns: HierarchicalConfiguration or None
    ...
    def set_order_weight(self):
        ...
        """
        for child in self.all_children():
            for rule in C.HIER_OPTIONS[self.host.os]['ordering']:
    ...
    def add_sectional_exiting(self):
        ...
        for child in self.all_children():
            for rule in C.HIER_OPTIONS[
                    self.host.os]['sectional_exiting']:
                if child.lineage_test(rule):
    ...
    def with_tags(self, tags, new_instance=None):
        ...
        """
        from netcli.hierarchical_configuration_root import HierarchicalConfigurationRoot
        if new_instance is None:
            new_instance = HierarchicalConfigurationRoot(self.host)
    ...
    def negate(self):
        """ Negate self.text """
        for rule in C.HIER_OPTIONS[
                self.host.os]['negation_negate_with']:
            if self.lineage_test(rule):
                self.text = rule['use']
                return self

        for rule in C.HIER_OPTIONS[
                self.host.os]['negation_default_when']:
            if self.lineage_test(rule):
    ...
    def config_to_get_to(self, target, delta=None):
        ...
        from netcli.hierarchical_configuration_root import HierarchicalConfigurationRoot
        if delta is None:
            delta = HierarchicalConfigurationRoot(self.host)
    ...
    def is_idempotent_command(self, other_children):
        ...
        for rule in C.HIER_OPTIONS[self.host.os][
        ...
        for rule in C.HIER_OPTIONS[
                self.host.os]['idempotent_commands']:
            if self.lineage_test(rule, True):
                for other_child in other_children:
    ...
    def sectional_overwrite_check(self):
        ...
        for rule in C.HIER_OPTIONS[
                self.host.os]['sectional_overwrite']:
            if self.lineage_test(rule):
                return True
        return False
