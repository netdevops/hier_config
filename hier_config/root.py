...
from hier_config import HierarchicalConfiguration
...
class HierarchicalConfigurationRoot(HierarchicalConfiguration):
    ...
    See ./conf/hierarchical_configuration_tags_* and ./conf/hierarchical_configuration_options_*
    ...
    def load_from_string(self, config_text):
        ...
        for sub in C.HIER_OPTIONS[self.host.os]['full_text_sub']:
            config_text = re.sub(
                sub['search'],
                sub['replace'],
                config_text)
        ...
        for line in config_text.splitlines():
            ...
            for sub in C.HIER_OPTIONS[self.host.os]['per_line_sub']:
                line = re.sub(
                    sub['search'],
                    sub['replace'],
                    line)
            ...
            for expression in C.HIER_OPTIONS[
                    self.host.os]['indent_adjust']:
    ...
    def all_children_sorted_with_lineage_rules(self, rules):
        ...
        Yields: instances of HierarchicalConfiguration that match one of the lineage rules
