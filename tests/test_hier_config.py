...
def test_hash(platform_a: Platform) -> None:
    config = get_hconfig_from_simple(platform_a, ("interface 1/1", "  untagged vlan 5"))
    assert hash(config)
...
def test_dump_and_load_from_dump_and_compare(platform_a: Platform) -> None:
    hier_pre_dump = get_hconfig_for_platform(platform_a)
...
def test_child_deep2() -> None:
    config = get_hconfig_for_platform(Platform.CISCO_IOS)
...
def test_add_children(platform_a: Platform) -> None:
    ...
    hier2 = get_hconfig_for_platform(platform_a)
...
def test_config_to_get_to(platform_a: Platform) -> None:
    running_config_hier = get_hconfig_for_platform(platform_a)
...
def test_future_config(platform_a: Platform) -> None:
    running_config = get_hconfig_for_platform(platform_a)
...
def test_unified_diff() -> None:
    ...
    config_a = get_hconfig_for_platform(platform)
    config_b = get_hconfig_for_platform(platform)
    # deep differences
