...
def _load(lines: tuple[str, ...]) -> HConfig:
    return HConfig.from_lines(Platform.RUCKUS_FASTIRON, lines)
...
def _ordered_remediation(
    ...
    return workflow.remediation_config.to_lines()
...
def test_per_line_sub_strips_banner_noise() -> None:
    ...
    assert config.to_lines() == ("hostname SW01",)
...
def test_stack_config_is_not_remediated() -> None:
    ...
    generated_config = HConfig.from_text(Platform.RUCKUS_FASTIRON, "hostname SW01\n")
    assert running_config.to_lines() == ("hostname SW01",)
    assert not running_config.remediation(generated_config).to_lines()
...
def test_split_vlan_blocks_do_not_produce_a_diff() -> None:
    ...
    assert not from_device.remediation(from_template).to_lines()
...
def test_single_port_change_only_touches_that_port() -> None:
    ...
    assert remediation.to_lines() == ("vlan 101", " no tagged ethe 1/1/4")
...
def test_vlan_header_changes_never_negate_the_vlan(
    ...
    remediation = running_config.remediation(generated_config)
    assert remediation.to_lines() == ("vlan 20", expected)
...
def test_vlan_negation_drops_the_inline_name() -> None:
    ...
    remediation = running_config.remediation(generated_config)
    assert "no vlan 101" in remediation.to_lines()
    assert "no vlan 101 name USERS by port" not in remediation.to_lines()
...
def test_vlan_rename_does_not_delete_the_vlan() -> None:
    ...
    remediation = running_config.remediation(generated_config)
    assert remediation.to_lines() == ("vlan 101", " vlan 101 name NEW by port")
    _assert_rollback_restores(running_config, remediation)
...
def test_lag_negation_drops_the_static_id() -> None:
    ...
    generated_config = _load(("hostname SW01",))
    remediation = running_config.remediation(generated_config)
    assert 'no lag "CORE_UPLINK"' in remediation.to_lines()
...
def test_port_name_negation_takes_no_argument() -> None:
    ...
    remediation = running_config.remediation(generated_config)
    assert " no port-name" in remediation.to_lines()
...
def test_round_trip_against_real_configs(
    fastiron_running_config: str,
    fastiron_generated_config: str,
) -> None:
    """Round trip over a stacked ICX 6450 pair running the L2 (`S`) image."""
    running_config = HConfig.from_text(
        Platform.RUCKUS_FASTIRON, fastiron_running_config
    )
    generated_config = HConfig.from_text(
        Platform.RUCKUS_FASTIRON, fastiron_generated_config
    )
...
def test_round_trip_against_real_l3_configs(
    fastiron_l3_running_config: str,
    fastiron_l3_generated_config: str,
) -> None:
    ...
    """
    running_config = HConfig.from_text(
        Platform.RUCKUS_FASTIRON, fastiron_l3_running_config
    )
    generated_config = HConfig.from_text(
        Platform.RUCKUS_FASTIRON,
        fastiron_l3_generated_config,
    )
