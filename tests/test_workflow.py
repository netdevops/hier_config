...
def test_remediation_config_driver_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="The running and generated configs must use the same driver."):
...
def test_rollback_config_reverts_changes(wfr: WorkflowRemediation) -> None:
    ...
    rollback_config = wfr.rollback_config
    rollback_text = "\n".join(line.cisco_style_text() for line in rollback_config.all_children_sorted())
